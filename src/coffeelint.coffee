...
class LexicalLinter
    ...
    setupPlugins: (plugins) ->
        @plugins = []
        for rule, ClassName of plugins
            level = @config[rule].level
            if level in ['error', 'warn']
                @plugins.push new ClassName this, @config
...
_plugins = {}
coffeelint.registerPlugin = (PluginClass) ->
    throw new Error "Missing rule name" unless PluginClass::rule_name?
    _plugins[PluginClass::rule_name] = PluginClass
